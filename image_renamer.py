...
def metadata_extractor(image):
    ...
    return {'fichier': image, 'temps': temps, 'date_formatee': date_formatee, 'extension': extension}

def image_renamer(dossier_images, dossier_sortie):
    liste_images = [f"{dossier_images}/{fichier}" for fichier in os.listdir() if
                    re.match(r'.*\.(.*)$', fichier).group(1).lower() in ['png', 'jpeg', 'jpg']]

    images = {fichier: metadata_extractor(fichier) for fichier in liste_images}
    ...
    for image in images_ordonnees:
        if date_retard and date_retard == images[image]['date_formatee']:
            ...
            increment = 1
        shutil.copy(image,
                    f"{dossier_sortie}/{images[image]['date_formatee']}_image_{str(increment).zfill(3)}.{images[image]['extension']}")
...
if __name__ == '__main__':
    ...
    image_renamer(args.dossier_images, dossier_sortie)
